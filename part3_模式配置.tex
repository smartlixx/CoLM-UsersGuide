...
值得注意的是，一个进程组包含一个IO进程和多个Worker进程，进程组的个数为模式使用的总IO进程和Worker进程数之和除以DEF\_PIO\_groupsize. 实际运行时，当进程组的数量大于数据块的数量时（DEF\_PIO\_groupsize设置过小），会产生闲置的进程组，因此，模式将进程组的数量设置为数据块的数量，使用总IO进程和Worker进程数之和除以数据块的数量重新计算一个进程组中进程的数量，以替代DEF\_PIO\_groupsize.
...
\texttt{DEF\_file\_mesh\_filter} & 字符串 & 使用此文件，可从模拟区域中屏蔽掉部分子区域或格点。文件中数据变量名为`mesh\_filter'，变量值为正表示模拟，否则表示不模拟。`mesh\_filter'的网格可与模式单元网格不一致 \\
\midrule
\textbf{2. 次网格单元} & & \\
\textbf{变量名} & \textbf{数据类型} & \textbf{说明} \\
...
对使用\texttt{USE\_SITE\_\allowbreak\$data}指定来源的其他变量，若SITE\_fsitedata文件中不存在该变量，则使用由全球基础地表数据中读取的值。
...
\texttt{DEF\_Forcing\%timelog(8)} & 字符串数组 & 强迫变量记录的时间方式。`$instant$'表示瞬时值，`$forword$'表示该时间点向前记录的1个时间步长的平均值，`$backward$'表示该时间点向后记录的1个时间步长的平均值 \\
...
\texttt{DEF\_Forcing\%tintalgo(8)} & 字符串数组 & 大气驱动变量在模式中的时间维插值方案。按照温度、比湿、气压、降水率、纬向风速、经向风速、下行短波辐射、下行长波辐射的顺序依次设置。目前默认的方案为温度、比湿、气压、风速和下行长波辐射采用线性插值方案(`$linear$')，降水率采用临近时刻插值方案(`$nearest$')或者均一取值方案(`$uniform$`)，下行短波辐射采用根据太阳高度角的余弦插值方案(`$coszen$')。缺失的变量设为NULL \\
...
CONC\_O2\_UNSAT & 非淹没区的土壤氧气浓度 & {[}soil, lon, lat, time{]} & \unit{mol.m^{-3}} \\
O2\_DECOMP\_DEPTH\_UNSAT & 非淹没区的土壤氧气消耗 & {[}soil, lon, lat, time{]} & \unit{mol.m^{-3}.s^{-1}} \\
...
plantdate\_irrigated\_trop\_corn & 灌溉热带玉米播种日期 & {[}lon, lat, time{]} & \unit{day} \\
plantdate\_rainfed\_trop\_soybean & 雨养热带大豆播种日期 & {[}lon, lat, time{]} & \unit{day} \\
plantdate\_irrigated\_trop\_soybean & 灌溉热带大豆播种日期 & {[}lon, lat, time{]} & \unit{day} \\
plantdate\_unmanagedcrop & 无管理农田播种日期 & {[}lon, lat, time{]} & \unit{day} \\
cropprodc\_rainfed\_temp\_corn & 雨养温带玉米瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_temp\_corn & 灌溉温带玉米瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_spwheat & 雨养春小麦瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_spwheat & 灌溉春小麦瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_wtwheat & 雨养冬小麦瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_wtwheat & 灌溉冬小麦瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_temp\_soybean & 雨养温带大豆瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_temp\_soybean & 灌溉温带大豆瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_cotton & 雨养棉花瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_cotton & 灌溉棉花瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_rice & 雨养水稻瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_rice & 灌溉水稻瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_sugarcane & 雨养甘蔗瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_irrigated\_sugarcane & 灌溉甘蔗瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
cropprodc\_rainfed\_trop\_corn & 雨养热带玉米瞬时产量 & {[}lon, lat, time{]} & \unit{gC.m^{-2}.s^{-1}} \\
